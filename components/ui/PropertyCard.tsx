...
export function PropertyCard({ property }: PropertyCardProps) {
  ...
  useEffect(() => {
    setIsFavorite(favoritesService.isFavorite(property.id))
  ...
  return (
    <Link href={`/property/${property.id}`} className="block h-full group">
      <article className="bg-white dark:bg-white/5 rounded-xl overflow-hidden shadow-card hover:shadow-soft transition-all duration-300 cursor-pointer h-full flex flex-col">
          ...
          <button 
            className={`absolute top-3 right-3 p-2 rounded-full transition-colors ${
              isFavorite 
                ...
                : 'bg-white/90 dark:bg-black/50 text-nordic-dark hover:bg-mosque hover:text-white'
            }`}
